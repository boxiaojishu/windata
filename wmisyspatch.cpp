...
int Getuninstallpatch(TCHAR *ppatchlist)
{
    HRESULT hr;
    ...
    VARIANT_BOOL isinstall;

    results->get_Updates(&updateList);
    updateList->get_Count(&updateSize);

    //wcout << "update num:" << updateSize << endl;
    ...
    for (LONG i = 0; i < updateSize; i++)
    {
        ...
        updateItem->get_IsInstalled(&isinstall);
        updateItem->get_Description(&desc);
        updateItem->get_SupportUrl(&url);
        updateItem->get_HandlerID(&handle);
        COleDateTime odt;
        odt.m_dt = retdate;
        USES_CONVERSION;
        if (i == 0)
        {
            _snwprintf_s(ppatchlist, 102400,102400 ,_T("%s,%s,%s,%s,%s,%d"), updateName, odt.Format(_T("%Y-%m-%d %H:%M:%S")), desc, url, handle, isinstall);
        }
        else {
